...
const Auction = () => {
  ...
  async function getSpace() {
    const url =
      "http://localhost:3001/api/auctionSpace/onlyAuction?id=" + productId;
    const tokenStr = localStorage.getItem("token");
    const headers = { Authorization: "Bearer " + tokenStr };
    axios.get(url, { headers }).then((res) => {
      if (res.status === 404 || !res) {
  ...
  const bidTable = viewAll
    ? bids.map((bid) => <DisplayRow bid={bid} />)
    : bids.slice(0, 5).map((bid) => <DisplayRow bid={bid} />);
  ...
  return (
    <div>
      ...
      <h1>
        {" "}
        Auction Space for{" "}
        <a className={styles.link} href={`/productPage?id=${productId}`}>
          {" "}
          {auction.productName}{" "}
        </a>{" "}
      </h1>
        ...
        <div>
          <div className={styles.container}>
            <div className={styles.element}>
              <h6> Current max price: LKR {auction.prodCurrentPrice} </h6>
            </div>
            <div className={styles.element}>
              <h6> Base Price: LKR {auction.prodBasePrice} </h6>
            </div>
            <div className={styles.element}>
              <h6> Status: {auction.auctionStatus} </h6>
            </div>
            <div className={styles.element}>
              <h6> Sold To: {auction.soldTo ? auction.soldTo : "--"} </h6>
            </div>
          </div>
        </div>
        <div>
          <div className={styles.container}>
            <div className={styles.element}>
              <h6> Auction Start Time: {auction.startDateTime} </h6>
            </div>
            <div className={styles.element}>
              <h6>
                {" "}
                {auction.auctionLive ? (
                  <AuctionTimer productId={productId} />
                ) : (
                  "--"
                )}{" "}
              </h6>
            ...
            <div className={styles.element}>
              <h6> Auction End Time: {auction.endDateTime} </h6>
            </div>
          </div>
        </div>
        <Form onSubmit={addBid} method="POST">
          <input
            id="pr"
            type="text"
            name="price"
          ...
          <Button type="submit" className="btn btn-dark bid-btn">
            bid
          </Button>
        </Form>
        <div>
          <div className="AuctionTab">
            ...
            <button className="btn btn-primary bid-btn" onClick={changeClick}>
